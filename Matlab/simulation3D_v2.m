...
% end
% Data = [background; cluster];
%% generate (original) data - randomly distributed clusters with a constrain: distance between the centers of clusters >= 5r; distance from the borders to the centers of the clusters >= 2.5r; smooth borders (from erf function); generate exact number of points
background = rand(round(bkgdDens * (1000)^3),3) * (1E3); 
%centers = rand(125,3)*800+50;
...
Nbins = round(1.5*(size(Sr,1))^(1/3)); %can be adjusted to smooth the histogram
lim = 2.7*median(Sr(Sr<Inf)); %can be adjusted to cover the needed range
...
for j = 1:iter
    zer = false(size(ind));
    zer(:,j) = ind(:,j);
    [counts_r(j,:), ~] = hist(Sr(zer),centers);
end
toc;
MeanCounts = mean(counts_r);
StdCounts = std(counts_r);

%%   Plot curves, check the intersections manually and put them to the next section: 'thresh', 'thresh2'
...
legend('Data', 'Confidence envelope', 'Mean of random distribution');
